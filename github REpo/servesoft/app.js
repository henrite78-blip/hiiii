...
async function hydrateFromServer() {
  try {
    const response = await fetch('${API_BASE}/bootstrap.php', { headers: { Accept: 'application/json' } });
...
async function checkAuth() {
  try {
    const response = await fetch('${API_BASE}/api_auth.php?action=check');
...
async function register(formData) {
  try {
    const response = await fetch('${API_BASE}/api_auth.php?action=register', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
async function logout() {
  try {
    await fetch('${API_BASE}/api_auth.php?action=logout', { method: 'POST' });
    store.setState((state) => {
