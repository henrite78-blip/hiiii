...
try {
    switch ($action) {
        case 'get_menu':
            ...
            // Remove 'r' prefix if present (e.g., 'r1' becomes 1)
            if (is_string($restaurantId) && strpos($restaurantId, 'r') === 0) {
                $restaurantId = intval(substr($restaurantId, 1));
            } else {
                $restaurantId = intval($restaurantId);
            }
            
            // Debug: Log the restaurant ID
            error_log("Getting menu for restaurant ID: " . $restaurantId);
